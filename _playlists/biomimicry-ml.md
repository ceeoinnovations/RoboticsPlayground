---
layout: post
title: Machine Learning Biomimicry Challenge
displayimg: web/img/frog.png
description: Challenge to build a robot that mimics an animal and train it to recognize and collect food while avoiding poison.
---


===
title
: Teacher Overview

description
: This teacher guide gives an overview of machine learning and the machine learning activities, including instructions to prepare the hub to work with the MINDSTORMS App.

placematlinks
: [Teacher Overview](https://drive.google.com/file/d/1upSx8TMDlMuVSofedxhu5K8AlB58zw9t/view?usp=sharing)

placematpic
: ../web/img/biomimic.png


===

===
title
: Activity 1: Biomimicry Remote Control

description
: Buid a robot that mimics an animal and use an Xbox or PlayStation controller to control it using the LEGO MINDSTORMS Inventor App. 

placematlinks
: [Placemat Link](https://docs.google.com/presentation/d/1e754dqEZKxobHctskeDfe54Nbo0aj8Ib0WxcDTrK6hE/edit?usp=sharing)

placematpic
: ../web/img/biomimic1.png

===

===
title
: Activity 2: Biomimicry Food Collection

description
: Build a mechanism for your animal robot to pick up and transport food. 

placematlinks
: [Placemat Link](https://docs.google.com/presentation/d/10ZnVz5BU-u22ssLBNm4ac_PXMflua2WsEjpZKoE6fkU/edit?usp=sharing)

placematpic
: ../web/img/biomimic2.png

===

===
title
: Activity 3: Biomimicry Food Identification

description
: Use machine learning to teach your robot to identify food and poison. 

placematlinks
: [Placemat Link](https://docs.google.com/presentation/d/1qC0CIfIA267NXJEa0a_vs1K30Y9ihvXgyOVmAulABJk/edit?usp=sharing)

placematpic
: ../web/img/biomimic3.png

===

===
title
: Activity 4: Biomimicry Collection Challenge

description
: How much food can your bio-inspired robot bring collect in five minutes?

placematlinks
: [Placemat Link](https://docs.google.com/presentation/d/1N2BvEdvkvcq2VDd_Uy32xy59xLo4v3myqv_lnzBHYk4/edit?usp=sharing)

placematpic
: ../web/img/biomimic4.png

===
